...
export default function Sidebar() {
    ...
    const navItems = [
        ...
        { name: "Aprovações", href: "/admin/aprovacoes", icon: <IconClipboardCheck /> },
        { name: "Apontamentos", href: "/admin/apontamento", icon: <IconClipboardList /> },
        { name: "Chat", href: "/admin/chat", icon: <IconChat /> },
        { name: "Relatórios", href: "/admin/relatorios", icon: <IconChartBar /> },
        { name: "Usuários", href: "/admin/usuarios", icon: <IconUserGroup /> },
    ];
    ...
    return (
        <aside className="w-64 bg-[#121212] text-gray-300 flex-shrink-0 flex flex-col justify-between hidden md:flex">
                        ...
                        return (
                            <a key={item.name} href={item.href}
                               className={`flex items-center gap-4 p-3 rounded-lg transition-colors duration-200 
                                ${isActive ? 'bg-red-800/50 text-white font-semibold border-l-4 border-red-500' : 'border-l-4 border-transparent text-gray-400 hover:bg-gray-700/50'}`}
                            >
                               {item.icon}
                               <span>{item.name}</span>
                            </a>
            ...
            <div className='flex flex-col'>
                ...
                <div className="p-1 border-t border-gray-700/50">
                    <div ref={dropUpRef} className="relative">
                         {isDropUpOpen && (
                             <div className="absolute bottom-full left-0 right-0 mb-4 bg-[#1f1f1f] rounded-md shadow-lg border border-black">
                                 <ul className="p-2 space-y-1">
                                     {portalLinks.map(link => (
                                         <li key={link.name}>
                                            <a href={link.href} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 w-full p-2 text-sm rounded-md hover:bg-gray-700">{link.icon}{link.name}</a>
                                        </li>
                                     ))}
                                 </ul>
                             </div>
                         )}
                         <button onClick={() => setDropUpOpen(!isDropUpOpen)} className="w-full flex items-center justify-between p-3 text-left rounded-lg hover:bg-gray-700/50 transition-colors">
                            <span>Outros Portais</span><IconChevronUp className={`transition-transform duration-300 ${isDropUpOpen ? 'rotate-180' : ''}`} />
                        </button>
                    </div>
                </div>
                ...
                <div className="p-4 border-t border-gray-700/50">
                    <div>
                        <h4 className="font-semibold text-sm text-gray-400">Escola SENAI</h4>
                        <p className="text-xs text-gray-500">"Armando de Arruda Pereira"</p>
                    </div>
                    <div className="pt-2 mt-1 space-y-2 text-xs">
                        <p className="text-gray-500">
                            <strong className="text-gray-400 font-medium">Telefone:</strong> (11) 4227-7450
                        </p>
                        <p className="text-gray-500">
                            <strong className="text-gray-400 font-medium">Email:</strong> senai.scs@sp.senai.br
                        </p>
                    </div>
                    <div className="pt-4 mt-4 flex justify-center items-center gap-4 border-t border-gray-700/50">
                        <a href="https://youtu.be/xvFZjo5PgG0?si=vbf1lNNazWvn-6Pr" target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-red-500 transition-colors" title="YouTube"><IconYouTube/></a>
                        <a href="" target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-pink-500 transition-colors" title="Instagram"><IconInstagram/></a>
                        <a href="" target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-blue-500 transition-colors" title="Facebook"><IconFacebook/></a>
                        <a href="" target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-white transition-colors" title="Twitter (X)"><IconTwitter/></a>
                    </div>
                     <div className="mt-4 pt-2 text-center">
                         <p className="text-xs text-gray-600">© {new Date().getFullYear()} Zelos</p>
                     </div>
                </div>
            </div>
        </aside>
